...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const getResource = async url => {
    let res = await fetch(url); //получаем ответ от сервера
    //обработаем как json формат

    if (!res.ok) {
      //если в запросе что-то пошло нет так
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); //entries - превратить обьект в массив с массивами
